...
int main() {
    ...
    expect_true(balanced.accumulation_reset_translation == 0.02,
        "balanced accumulation translation");
    ...
    expect_true(realtime.accumulation_reset_translation == 0.05,
        "realtime accumulation translation");
    ...
    expect_true(realtime_default.samples_per_pixel == balanced.samples_per_pixel,
        "default spp matches balanced");
    expect_true(realtime_default.max_bounces == balanced.max_bounces,
        "default bounces match balanced");
    expect_true(realtime_default.enable_denoise == balanced.enable_denoise,
        "default denoise matches balanced");
    expect_true(realtime_default.rr_start_bounce == balanced.rr_start_bounce,
        "default rr start matches balanced");
    expect_true(realtime_default.accumulation_reset_rotation_deg
                    == balanced.accumulation_reset_rotation_deg,
        "default rotation matches balanced");
    expect_true(realtime_default.accumulation_reset_translation
                    == balanced.accumulation_reset_translation,
        "default translation matches balanced");
    ...
    expect_true(viewer_preview.accumulation_reset_rotation_deg == 2.0,
        "viewer preview accumulation rotation");
    expect_true(viewer_preview.accumulation_reset_translation == 0.05,
        "viewer preview accumulation translation");
    ...
    expect_true(viewer_converge.accumulation_reset_rotation_deg == 2.0,
        "viewer converge accumulation rotation");
    expect_true(viewer_converge.accumulation_reset_translation == 0.05,
        "viewer converge accumulation translation");

    const rt::RenderProfile viewer_default = rt::viewer::default_viewer_profile();
    expect_true(viewer_default.samples_per_pixel == viewer_preview.samples_per_pixel,
        "viewer default spp matches preview");
    expect_true(viewer_default.max_bounces == viewer_preview.max_bounces,
        "viewer default bounces match preview");
    expect_true(viewer_default.enable_denoise == viewer_preview.enable_denoise,
        "viewer default denoise matches preview");
    expect_true(viewer_default.rr_start_bounce == viewer_preview.rr_start_bounce,
        "viewer default rr start matches preview");
    expect_true(viewer_default.accumulation_reset_rotation_deg
                    == viewer_preview.accumulation_reset_rotation_deg,
        "viewer default rotation matches preview");
    expect_true(viewer_default.accumulation_reset_translation
                    == viewer_preview.accumulation_reset_translation,
        "viewer default translation matches preview");
    ...
    expect_true(named_quality->samples_per_pixel == quality.samples_per_pixel,
        "quality profile spp resolved");
    const std::optional<rt::RenderProfile> named_balanced =
        rt::render_profile_from_name("balanced");
    expect_true(named_balanced.has_value(), "balanced profile resolved");
    expect_true(named_balanced->samples_per_pixel == balanced.samples_per_pixel,
        "balanced profile spp resolved");
    const std::optional<rt::RenderProfile> named_realtime =
        rt::render_profile_from_name("realtime");
    expect_true(named_realtime.has_value(), "realtime profile resolved");
    expect_true(named_realtime->samples_per_pixel == realtime.samples_per_pixel,
        "realtime profile spp resolved");
    expect_true(!rt::render_profile_from_name("bad").has_value(), "unknown profile rejected");
    expect_true(rt::render_profile_name(rt::RenderProfile::realtime_default())
                    == std::string("balanced"),
        "default profile name");
    expect_true(rt::render_profile_name(rt::RenderProfile {}) == std::string("default"),
        "custom profile name");
    return 0;
}
