...
double compute_average_luminance_from_beauty(const std::vector<float>& rgba) {
    ...
    for (std::size_t i = 0; i + 3 < rgba.size(); i += 4) {
        const double r =
            std::clamp(std::sqrt(std::max(0.0, static_cast<double>(rgba[i + 0]))), 0.0, 0.999);
        const double g =
            std::clamp(std::sqrt(std::max(0.0, static_cast<double>(rgba[i + 1]))), 0.0, 0.999);
        const double b =
            std::clamp(std::sqrt(std::max(0.0, static_cast<double>(rgba[i + 2]))), 0.0, 0.999);
        sum += (r + g + b) / 3.0;
    }
...
int main() {
    ...
    const rt::RealtimeFrameSet first =
        pipeline.render_profiled_smoke_frame(2, rt::RenderProfile::balanced());
    ...
    expect_true(first.frames[0].radiance.average_luminance > 0.01,
        "camera 0 radiance should be non-black");
    ...
    const rt::RealtimeFrameSet second =
        pipeline.render_profiled_smoke_frame(2, rt::RenderProfile::balanced());
    ...
    const rt::RealtimeFrameSet after_jump =
        pipeline.render_profiled_smoke_frame(2, rt::RenderProfile::balanced());
    expect_true(after_jump.frames[0].history_length == 2,
        "camera 0 resumes accumulation after jump");
    expect_true(after_jump.frames[1].history_length == 2,
        "camera 1 resumes accumulation after jump");
    return 0;
}
