...
if(NOT DEFINED EXPECT_DISTINCT_CAMERA_LUMINANCE)
    set(EXPECT_DISTINCT_CAMERA_LUMINANCE OFF)
endif()
...
if(EXPECT_DISTINCT_CAMERA_LUMINANCE)
    string(REGEX MATCHALL "\"average_luminance\" *: *([-0-9.eE+]+)" luminance_matches "${json_flat}")
    if(NOT luminance_matches)
        message(FATAL_ERROR "per-camera records missing average_luminance fields")
    endif()

    set(luminance_values)
    foreach(luminance_match IN LISTS luminance_matches)
        string(REGEX REPLACE ".*: *([-0-9.eE+]+).*" "\\1" luminance_value "${luminance_match}")
        list(APPEND luminance_values "${luminance_value}")
    endforeach()

    list(REMOVE_DUPLICATES luminance_values)
    list(LENGTH luminance_values distinct_luminance_count)
    if(distinct_luminance_count LESS 2)
        message(FATAL_ERROR "expected at least two distinct per-camera luminance values")
    endif()
endif()
