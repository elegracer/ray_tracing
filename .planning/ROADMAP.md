...
- [ ] **Phase 3: OpenPBR Core And Physical BSDFs** — the official OpenPBR 1.1.1 authoring contract, MaterialX texture nodes, connected inputs, explicit color spaces, and all legacy material projections are complete; matched evaluate/sample/PDF behavior, physical gates, and compatibility images remain.
...
Implement the first shared CPU/GPU OpenPBR evaluator slice for base, specular, transmission, emission, opacity, thin-walled, normal, and tangent semantics with matched evaluate/sample/PDF behavior and energy/PDF invariants; keep advanced lobes and the OpenUSD SDK frontend behind their later phase gates.
