...
- [ ] **Phase 3: OpenPBR Core And Physical BSDFs** — the official OpenPBR 1.1.1 authoring contract and shared CPU/GPU evaluator now run through an opt-in SceneIR v2 production path with matched sampling, direct response, emission, compact GPU sidecar storage, and a deterministic linear reference gate; connected texture/color evaluation and compatibility images remain.
...
Implement PBR-04 by evaluating supported SceneIR v2 MaterialX color3 connections with explicit color-space conversion in the shared CPU/GPU parameter compiler, then add PBR-05 legacy compatibility images before considering a default-path migration. Keep advanced lobes and the OpenUSD SDK frontend behind their later phase gates.
