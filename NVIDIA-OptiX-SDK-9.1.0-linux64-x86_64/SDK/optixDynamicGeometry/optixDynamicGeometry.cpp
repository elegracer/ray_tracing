...
void createPipeline( DynamicGeometryState& state )
{
    ...
    uint32_t max_dc_depth_traverse = 0;
    // This is 2 since the largest depth is IAS->GAS
    const uint32_t max_traversable_graph_depth = 2;
    OPTIX_CHECK( optixPipelineSetStackSizeFromCallDepths( state.pipeline, max_trace_depth, max_cc_depth, max_dc_depth_state,
                                                          max_dc_depth_traverse, max_traversable_graph_depth ) );
}
