...
// PCG Hashing function (used for fast on-device pseudorandom numbers for randomness in noise, https://www.pcg-random.org/)
__forceinline__ __device__ float hash( float2 st )
{
    uint64_t state = __float_as_uint( st.x );
    state          = ( state << 32 ) | __float_as_uint( st.y );

    state         = state * 6364136223846793005ULL + 1442695040888963407ULL;
    uint32_t word = ( ( state >> 18u ) ^ state ) >> 27u;
    uint32_t rot  = state >> 59u;
    word          = ( word >> rot ) | ( word << ( ( -rot ) & 31 ) );
