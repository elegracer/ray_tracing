...
{
public:
    SUTILAPI bool wheelEvent(int dir); // Prefer zoom()

    SUTILAPI void startTracking(int x, int y);
    SUTILAPI void updateTracking(int x, int y, int canvasWidth, int canvasHeight, float speed=1.0f);
    SUTILAPI void zoom(int direction, float speed=1.0f);
    SUTILAPI float moveSpeed() const { return m_moveSpeed; } // UNUSED
    SUTILAPI void setMoveSpeed(const float& val) { m_moveSpeed = val; } // UNUSED
...
private:
    ...
    float        m_moveSpeed                = 1.0f; // UNUSED
    float        m_rollSpeed                = 0.5f; // UNUSED
