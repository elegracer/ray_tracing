...
void Trackball::updateTracking(int x, int y, int /*canvasWidth*/, int /*canvasHeight*/, float speed)
{
    ...
    m_prevPosX = x;
    m_prevPosY = y;
    m_latitude = radians(std::min(89.0f, std::max(-89.0f, degrees(m_latitude) + 0.5f*speed*deltaY)));
    m_longitude = radians(fmod(degrees(m_longitude) - 0.5f*speed*deltaX, 360.0f));
...
void Trackball::zoom(int direction, float speed)
{
    float zoom = (1.f + (m_zoomMultiplier - 1.f) * speed);
    if (direction > 0) zoom = 1.f / zoom;
